...
def ParseCmdLineArgs(cross_prefix: str) -> Tuple[
    ...
    if cross_prefix:
        ...
        riscv = Matcher(r'ELF 32-bit LSB.*RISC-V')
        ...
        if x86.search(binarySignature):
            ...
            stackUsagePattern = Matcher(
                r'^.*sub.*sp, (#[0-9][0-9]*)')
        elif riscv.search(binarySignature):
            objdump = 'riscv64-elf-objdump'
            nm = 'riscv64-elf-nm'
            functionNamePattern = Matcher(r'^(\S+) <([a-zA-Z0-9_]+?)>:')
            callPattern = Matcher(r'^.*jalr?\s+\S+\s+<([a-zA-Z0-9_]+)>')
            stackUsagePattern = Matcher(r'^.*add.*sp,sp,(-[0-9]+)')
        else:
